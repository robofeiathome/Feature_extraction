...
cats = (
    'blouse', 'top', 'sweater', 'cardigan', 'jacket', 'vest', 'pants', 'shorts', 'skirt', 'coat', 'dress', 'jumpsuit',
    'cape', 'glasses', 'hat', 'hair accessory', 'tie', 'glove', 'watch', 'belt', 'leg warmer', 'tights, stockings',
    'sock',
    'shoe', 'bag, wallet', 'scarf', 'umbrella', 'hood', 'collar', 'lapel', 'epaulette', 'sleeve', 'pocket', 'neckline',
    'buckle', 'zipper', 'applique', 'bead', 'bow', 'flower', 'fringe', 'ribbon', 'rivet', 'ruffle', 'sequin', 'tassel')
...
class FeatureExtractor:
    def __init__(self):
        ...
        self.main_dir = rospack.get_path('features_pkg')
        self.directory = self.main_dir + '/src/'
    ...
    def visualize_predictions(self, image, outputs, threshold=0.8):
        ...
        way = glob.glob(self.directory + 'results/*')
        for py_file in way:
            try:
                os.remove(py_file)
            except OSError as e:
    ...
    def delta_e_cie76(self, color1, color2):
        ...
        return ((2 + (r_mean / 256)) * delta_r ** 2 + 4 * delta_g ** 2 + (
                    2 + ((255 - r_mean) / 256)) * delta_b ** 2) ** 0.5
    ...
    def main(self):
        ...
        cv2.imwrite(self.directory + 'base/img.png', frame)
        path = self.directory + 'base/img.png'

        self.inferencing(path)
        out = ''

        if self.directory + 'results/hat.png' not in glob.glob(self.directory + 'results/*') and \
        self.directory + 'results/cap.png' not in glob.glob(self.directory + 'results/*') \
        and self.directory + 'results/hair accessory.png' not in glob.glob(self.directory + 'results/*'):
